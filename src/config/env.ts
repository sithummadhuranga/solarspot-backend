...
export const config = {
  ...
  EMAIL_USER: process.env.EMAIL_USER as string,             // REQUIRED for email
  EMAIL_PASS: process.env.EMAIL_PASS as string,             // REQUIRED for email
  EMAIL_FROM_NAME: process.env.EMAIL_FROM_NAME ?? 'SolarSpot',
  EMAIL_FROM_ADDRESS: process.env.EMAIL_FROM_ADDRESS ?? 'no-reply@solarspot.app',
  // DEV ONLY — when true, writes email HTML to console/tmp instead of sending
  EMAIL_PREVIEW: process.env.EMAIL_PREVIEW === 'true',

  // ── Third-party APIs ─────────────────────────────────────
  OPENWEATHER_API_KEY: process.env.OPENWEATHER_API_KEY as string,  // REQUIRED for weather
